...
# ===== S13 研究内容(两项) =====
s=blank(); band(s,"主要研究内容(两项)")
bullets(body(s),[
 (0,"一套完整的专用 harness",True),
 (1,"将“由编码智能体生成 eBPF 防御并保证其可用、安全”的全过程封装为完整的领域专用 harness:涵盖生成、编译、内核 verifier 校验、沙箱测试、加载与运行时反馈"),
 (1,"解决生成质量(可编译、通过 verifier、不误伤正常流量)与运行安全(异常时自动回滚)"),
