...
# ===== S8 挑战 =====
s=blank(); band(s,"关键挑战(工程层面)")
bullets(body(s),[
 (0,"可靠性:智能体生成的内核代码若存在缺陷,可能导致网络中断或误伤正常流量 → 先验证安全、再上线,异常可回滚",True),
 (0,"生成质量:智能体生成的 eBPF 须能编译通过、通过 verifier,并有效拦截攻击而不误伤正常流量",True),
 (0,"评测:如何验证有效性——对已知与未知攻击均能防御,且性能开销可控",True),
])
...
bullets(body(s),[
 ...
 (0,"编码智能体(工程载体)",True),
 (1,"Claude Code/Cursor/Codex/Devin/SWE-agent:已能自主编写、修改与调试代码,但属通用工具、不含内核/eBPF 领域能力——本文为此设计专用 harness"),
...
# ===== S12 核心思路 =====
s=blank(); band(s,"总体思路:由智能体持续维护内核 eBPF 防御")
tf=body(s,top=1.5,height=1.6); p=tf.paragraphs[0]; p.line_spacing=1.25
r=p.add_run(); r.text="将“发现攻击 → 生成/修改 eBPF → 验证安全 → 加载 → 评估效果 → 迭代”组织为一个自动化闭环;防御能力以 eBPF 代码的形式持续积累于“防御库”中(本文称之为“防御演化”)。"; setrun(r,19,False,DARK)
bullets(body(s,top=3.6),[
 (0,"为何以代码(而非模型权重)为载体",True),
 (1,"逐条增补与修正,可累积、不遗忘;每条防御对应一段代码,可读、可审计"),
 (1,"运行于 XDP,线速执行;出错可回滚,并由 verifier 保证不致内核崩溃"),
])

# ===== S13 研究内容(三项) =====
s=blank(); band(s,"主要研究内容(三项)")
bullets(body(s),[
 (0,"系统构建:智能体与 eBPF 工具链闭环",True),
 (1,"在 PacketScope/Guarder 基础上,构建使智能体能够完成“生成 eBPF→编译→通过 verifier→沙箱测试→加载→读取遥测”的工具链与自动化闭环"),
 (0,"生成“可用且安全”的 eBPF 防御",True),
 (1,"解决生成质量(可编译、通过 verifier、不误伤正常流量)与运行安全(异常时自动回滚)——系统得以运行的关键工程"),
 (0,"面向真实协议攻击的防御与评测",True),
 (1,"以 PMTUD/NAT/IPID 等真实攻击构建攻防靶场,度量检测率、误报率与性能开销,并与人工规则及现有工具对比"),
])
...
s=blank(); band(s,"评测方法")
bullets(body(s),[
 (0,"思路:以真实协议攻击构建攻防靶场,由系统自动生成防御;并对未参与迭代的未知攻击单独评测",True),
 (0,"度量指标",True),
 (1,"检测率(是否成功拦截)、误报率(误伤正常流量)"),
 (1,"性能开销(XDP 吞吐 / 时延)、自动化程度(人工介入量)"),
 (0,"对比基线",True),
 (1,"人工编写的规则(现状)、Kgent/KEN(一次性合成)、Tetragon/Falco(人工策略)"),
])

# ===== S19 创新点 =====
s=blank(); band(s,"创新点")
bullets(body(s,top=1.6),[
 (0,"可运行的系统原型(而非概念设想):在 PacketScope/Guarder 上落地,由编码智能体自动维护内核 eBPF 防御,可演示、可复现",True),
 (0,"智能体编写内核代码的可靠流水线:生成→编译→verifier→沙箱→加载→回滚,保证智能体生成的 eBPF 可用,且不致引发内核崩溃",True),
 (0,"面向真实协议攻击的防御评测靶场:以 PMTUD/NAT/IPID 等真实攻击,给出检测率/误报/开销,并与人工规则、现有工具对比",True),
],base=19)
