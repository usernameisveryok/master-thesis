...
\textbf{研究内容二:让智能体写出"能用且安全"的 eBPF 防御。} 解决两个关键工程问题:\textbf{生成质量}(智能体生成的 eBPF 能编译通过、通过 verifier,且确实拦截攻击而不误伤正常流量)与\textbf{运行安全}(以编译/verifier 报错驱动自我修正,异常时自动回滚,保证不致网络中断或误伤正常流量)——这是系统能真正运行起来的前提。
